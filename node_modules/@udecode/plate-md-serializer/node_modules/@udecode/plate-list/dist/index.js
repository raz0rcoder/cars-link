...
const getListDeserialize = () => editor => {
  ...
  const lic = plateCore.getPlatePluginOptions(editor, ELEMENT_LIC);
  const ul = plateCore.getPlatePluginOptions(editor, ELEMENT_UL);
  const ol = plateCore.getPlatePluginOptions(editor, ELEMENT_OL);
  return {
    ...
    preInsert: () => {
      const liEntry = plateCommon.findNode(editor, {
        match: {
          type: li.type
        }
      });
...
const isListNested = (editor, listPath) => {
  ...
  const listParentNode = (_getParent = plateCommon.getParent(editor, listPath)) === null || _getParent === void 0 ? void 0 : _getParent[0];
  return (listParentNode === null || listParentNode === void 0 ? void 0 : listParentNode.type) === plateCore.getPlatePluginType(editor, ELEMENT_LI);
};

const getListTypes = editor => {
  return [plateCore.getPlatePluginType(editor, ELEMENT_OL), plateCore.getPlatePluginType(editor, ELEMENT_UL)];
};
...
const getHighestEmptyList = (editor, {
  ...
  const list = plateCommon.getAbove(editor, {
    at: liPath,
    match: {
  ...
  if (!diffListPath || !slate.Path.equals(listPath, diffListPath)) {
    if (listNode.children.length < 2) {
      const liParent = plateCommon.getAbove(editor, {
        at: listPath,
        match: {
...
const getListItemEntry = (editor, {
  ...
  if (_at) {
    ...
    if (node) {
      const listItem = plateCommon.getAbove(editor, {
        at: _at,
        match: {
          type: liType
        }
      ...
      if (listItem) {
        const list = plateCommon.getParent(editor, listItem[1]);
        return {
          list,
          listItem
        };
...
const moveListItemDown = (editor, {
  ...
  const previousSiblingItem = slate.Editor.node(editor, previousListItemPath);
...
const unwrapList = (editor, {
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    ...
    } while (plateCommon.getAbove(editor, {
      match: {
        type: getListTypes(editor),
        at
      }
...
const moveListItems = (editor, {
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    licPathRefsToMove.forEach(licPathRef => {
      ...
      if (!licPath) return;
      const listItem = plateCommon.getParent(editor, licPath);
      if (!listItem) return;
      const listEntry = plateCommon.getParent(editor, listItem[1]);
...
const toggleList = (editor, {
  ...
  if (plateCommon.isCollapsed(editor.selection) || !plateCommon.isRangeAcrossBlocks(editor)) {
    ...
    const commonEntry = slate.Node.common(editor, startPoint.path, endPoint.path);

    if (getListTypes(editor).includes(commonEntry[0].type) || commonEntry[0].type === plateCore.getPlatePluginType(editor, ELEMENT_LI)) {
...
const getListOnKeyDown = pluginKeys => editor => e => {
  ...
  const options = pluginKeys ? plateCore.mapPlatePluginKeysToOptions(editor, pluginKeys) : [];
  options.forEach(({
    type,
    hotkey
  }) => {
    ...
    if (plateCommon.isSelectionAtBlockStart(editor, {
      ...
      slate.Editor.withoutNormalizing(editor, () => {
        ...
        if (plateCommon.isFirstChild(listItem[1]) && !isListNested(editor, list[1])) {
          plateResetNode.getResetNodeOnKeyDown({
            rules: [{
              types: [plateCore.getPlatePluginType(editor, ELEMENT_LI)],
          ...
          })(editor)(plateResetNode.SIMULATE_BACKSPACE);
          moved = true;
          return;
        }
...
const selectionIsNotInAListHandler = editor => {
  const pointAfterSelection = slate.Editor.after(editor, editor.selection.focus.path);
  ...
  if (pointAfterSelection) {
    ...
    if (nextSiblingListRes) {
      ...
      if (!plateCommon.getText(editor, parentBlockEntity[1])) {
...
const selectionIsInAListHandler = (editor, res) => {
  ...
  if (!hasListChild(editor, listItem[0])) {
    ...
    if (!liWithSiblings) {
      ...
      const pointAfterListItem = slate.Editor.after(editor, listItem[1]);
...
const getListDeleteFragment = editor => {
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    ...
    const end = slate.Editor.end(editor, editor.selection);
    const liEnd = plateCommon.getAbove(editor, {
      at: end,
      match: {
        type: plateCore.getPlatePluginType(editor, ELEMENT_LI)
      }
    ...
    const liEndPathRef = liEndCanBeDeleted ? slate.Editor.pathRef(editor, liEnd[1]) : undefined;
    /**
     * Delete fragment and move end block children to start block
     */

    plateCommon.deleteFragment(editor);
    const start = slate.Editor.start(editor, editor.selection);
    const liStart = plateCommon.getAbove(editor, {
      at: start,
      match: {
        type: plateCore.getPlatePluginType(editor, ELEMENT_LI)
      }
    ...
    if (liEndPathRef) {
      ...
      const listStart = liStart && plateCommon.getParent(editor, liStart[1]);
...
const getListInsertBreak = editor => {
  ...
  const didReset = plateResetNode.getResetNodeOnKeyDown({
    rules: [{
      types: [plateCore.getPlatePluginType(editor, ELEMENT_LI)],
      defaultType: plateCore.getPlatePluginType(editor, plateCommon.ELEMENT_DEFAULT),
...
const getListInsertFragment = editor => {
  ...
  const trimList = listRoot => {
    ...
    const textEntries = Array.from(slate.Node.texts(listRoot));
    const commonAncestorEntry = textEntries.reduce((commonAncestor, textEntry) => slate.Path.isAncestor(commonAncestor[1], textEntry[1]) ? commonAncestor : slate.Node.common(listRoot, textEntry[1], commonAncestor[1]), // any list item would do, we grab the first one
  ...
  return fragment => {
    ...
    if (liEntry) {
      ...
      return slate.Transforms.insertNodes(editor, fragment.flatMap(node => trimList(node)), {
...
const normalizeListItem = (editor, {
  ...
  if (slate.Editor.isBlock(editor, firstLiChildNode) && !plateCommon.match(firstLiChildNode, {
    ...
    if (plateCommon.match(firstLiChildNode, {
      ...
      const parent = plateCommon.getParent(editor, listItem[1]);
...
const normalizeNestedList = (editor, {
  ...
  const parentNode = plateCommon.getParent(editor, path);
...
const getListNormalizer = (editor, {
  ...
  return ([node, path]) => {
    if (!plateCore.isElement(node)) return; // remove empty list
...
const createListPlugin = options => ({
  ...
  renderElement: plateCore.getRenderElement(KEYS_LIST),
...
const getTodoListDeserialize = () => editor => {
  const options = plateCore.getPlatePluginOptions(editor, ELEMENT_TODO_LI);
  return {
...
const createTodoListPlugin = () => ({
  ...
  renderElement: plateCore.getRenderElement(ELEMENT_TODO_LI),
  deserialize: getTodoListDeserialize(),
  onKeyDown: plateCommon.getToggleElementOnKeyDown(ELEMENT_TODO_LI)
});

exports.CLASS_TODO_LIST_CHECKED = CLASS_TODO_LIST_CHECKED;
exports.DEFAULTS_TODO_LIST = DEFAULTS_TODO_LIST;
...
exports.getListDeleteBackward = getListDeleteBackward;
exports.getListDeleteForward = getListDeleteForward;
exports.getListDeleteFragment = getListDeleteFragment;
exports.getListDeserialize = getListDeserialize;
exports.getListInsertBreak = getListInsertBreak;
exports.getListInsertFragment = getListInsertFragment;
exports.getListItemEntry = getListItemEntry;
exports.getListNormalizer = getListNormalizer;
exports.getListOnKeyDown = getListOnKeyDown;
exports.getListRoot = getListRoot;
exports.getListTypes = getListTypes;
exports.getTodoListDeserialize = getTodoListDeserialize;
