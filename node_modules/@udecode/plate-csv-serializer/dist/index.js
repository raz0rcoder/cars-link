...
const deserializeCSV = (editor, content, header = false, errorTolerance) => {
  ...
  const testCsv = papaparse.parse(content, {
    preview: 2
  });

  if (testCsv.errors.length === 0) {
    const csv = papaparse.parse(content, {
      header
    });
    if (!isValidCsv(csv.data, csv.errors, errorTolerance)) return;
    const paragraph = plateCore.getPlatePluginType(editor, plateCommon.ELEMENT_DEFAULT);
    const table = plateCore.getPlatePluginType(editor, plateTable.ELEMENT_TABLE);
    const th = plateCore.getPlatePluginType(editor, plateTable.ELEMENT_TH);
    const tr = plateCore.getPlatePluginType(editor, plateTable.ELEMENT_TR);
    const td = plateCore.getPlatePluginType(editor, plateTable.ELEMENT_TD);
    const ast = {
      type: table,
      children: []
    };
...
const withDeserializeCSV = ({
  ...
  editor.insertData = data => {
    ...
    const isEnabled = plateSerializer.isDeserializerEnabled(editor, plugins, csvDeserializerId);
...
const createDeserializeCSVPlugin = plateCore.getPlatePluginWithOverrides(withDeserializeCSV);

exports.createDeserializeCSVPlugin = createDeserializeCSVPlugin;
exports.csvDeserializerId = csvDeserializerId;
exports.deserializeCSV = deserializeCSV;
exports.withDeserializeCSV = withDeserializeCSV;
