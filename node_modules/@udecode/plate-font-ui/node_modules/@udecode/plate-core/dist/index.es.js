...
const getPlatePlugins = id => {
  ...
  return (_getPlateState = getPlateState(id)) === null || _getPlateState === void 0 ? void 0 : _getPlateState.plugins;
};
const usePlatePlugins = id => usePlateStore(useCallback(() => getPlatePlugins(id), [id]));
...
const pipeRenderElement = (editor, plugins = []) => {
  ...
  const propsOverriders = plugins.flatMap(plugin => castArray_1(plugin.overrideProps).flatMap(cb => {
    var _cb;

    return (_cb = cb === null || cb === void 0 ? void 0 : cb(editor)) !== null && _cb !== void 0 ? _cb : [];
  }));
...
const withPlate = ({
  ...
  editor = pipe(editor, ...withOverrides); // Default option type is the plugin key

  Object.keys(editor.options).forEach(key => {
    if (editor.options[key].type === undefined) editor.options[key].type = key;
...
const usePlateEffects = ({
  ...
  const storeEditor = usePlateEditorRef(id);
  const storeEnabled = usePlateEnabled(id);
  const storePlugins = usePlatePlugins(id); // Clear the state on unmount.
...
const usePlateEditorState = id => {
  usePlateStore(() => {
    ...
    return (_getPlateState = getPlateState(id)) === null || _getPlateState === void 0 ? void 0 : _getPlateState.keyChange;
  });
  return usePlateEditorRef(id);
};

const getPlateKeys = id => {
  var _getPlateState;

  return (_getPlateState = getPlateState(id)) === null || _getPlateState === void 0 ? void 0 : _getPlateState.pluginKeys;
};
const usePlateKeys = id => usePlateStore(useCallback(() => getPlateKeys(id), [id]));

const usePlateOptions = id => {
  var _usePlateEditorRef;

  return (_usePlateEditorRef = usePlateEditorRef(id)) === null || _usePlateEditorRef === void 0 ? void 0 : _usePlateEditorRef.options;
};

const getPlateSelection = id => {
  var _getPlateState;

  return (_getPlateState = getPlateState(id)) === null || _getPlateState === void 0 ? void 0 : _getPlateState.selection;
};
/**
 * Get the editor selection which is updated on editor change.
 */

const usePlateSelection = id => usePlateStore(useCallback(() => getPlateSelection(id), [id]));
...
const getRenderNodeProps = ({
  ...
  let props = { ..._props,
    nodeProps: (_ref = (_getNodeProps = getNodeProps === null || getNodeProps === void 0 ? void 0 : getNodeProps(_props)) !== null && _getNodeProps !== void 0 ? _getNodeProps : attributes) !== null && _ref !== void 0 ? _ref : {}
  };
...
const getEditableRenderElement = options => props => {
  ...
  const injectParentComponents = plugins.flatMap(o => {
    var _o$injectParentCompon;

    return (_o$injectParentCompon = o.injectParentComponent) !== null && _o$injectParentCompon !== void 0 ? _o$injectParentCompon : [];
  });
  const injectChildComponents = plugins.flatMap(o => {
    var _o$injectChildCompone;

    return (_o$injectChildCompone = o.injectChildComponent) !== null && _o$injectChildCompone !== void 0 ? _o$injectChildCompone : [];
  });
...
const getPlatePluginType = (editor, pluginKey) => {
  var _ref, _getPlatePluginOption;

  return (_ref = (_getPlatePluginOption = getPlatePluginOptions(editor, pluginKey).type) !== null && _getPlatePluginOption !== void 0 ? _getPlatePluginOption : pluginKey) !== null && _ref !== void 0 ? _ref : '';
};
...
export { DOM_HANDLERS, DefaultLeaf, EditablePlugins, EditorStateEffect, Plate, createHistoryPlugin, createInlineVoidPlugin, createReactPlugin, eventEditorStore, flatMapByKey, getEditableRenderElement, getEditableRenderLeaf, getEditorOptions, getEventEditorState, getInlineTypes, getPlateEditorRef, getPlateEnabled, getPlateEventId, getPlateId, getPlateKeys, getPlatePluginOptions, getPlatePluginType, getPlatePluginTypes, getPlatePluginWithOverrides, getPlatePlugins, getPlateSelection, getPlateState, getPlateValue, getRenderElement, getRenderLeaf, getRenderNodeProps, getSetStateByKey, getSlateClass, getStateById, getVoidTypes, isAncestor, isDescendant, isElement, isEventHandled, isText, mapPlatePluginKeysToOptions, mergeState, pipe, pipeDecorate, pipeHandler, pipeOnChange, pipeOverrideProps, pipeRenderElement, pipeRenderLeaf, plateStore, setEventEditorId, useEditableProps, useEditorRef, useEditorState, useEventEditorStore, usePlate, usePlateActions, usePlateEditorRef, usePlateEditorState, usePlateEffects, usePlateEnabled, usePlateEventId, usePlateKeys, usePlateOptions, usePlatePlugins, usePlateSelection, usePlateState, usePlateStore, usePlateValue, useSlateProps, withHistoryPersist, withInlineVoid, withPlate };
