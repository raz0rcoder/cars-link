...
const isListNested = (editor, listPath) => {
  ...
  const listParentNode = (_getParent = plateCore.getParent(editor, listPath)) === null || _getParent === void 0 ? void 0 : _getParent[0];
...
const getListTypes = editor => {
  return [plateCore.getPluginType(editor, ELEMENT_OL), plateCore.getPluginType(editor, ELEMENT_UL)];
};
...
const getHighestEmptyList = (editor, {
  ...
  const list = plateCore.getAbove(editor, {
    at: liPath,
    match: {
  ...
  if (!diffListPath || !slate.Path.equals(listPath, diffListPath)) {
    if (listNode.children.length < 2) {
      const liParent = plateCore.getAbove(editor, {
        at: listPath,
        match: {
...
const getListItemEntry = (editor, {
  ...
  if (_at) {
    ...
    if (node) {
      const listItem = plateCore.getAbove(editor, {
        at: _at,
        match: {
          type: liType
        }
      ...
      if (listItem) {
        const list = plateCore.getParent(editor, listItem[1]);
        return {
          list,
          listItem
        };
...
const getListRoot = (editor, at = editor.selection) => {
  if (!at) return;
  const parentList = plateCore.getAbove(editor, {
    at,
    match: {
...
const moveListItems = (editor, {
  ...
  const [...lics] = plateCore.getNodes(editor, {
    at,
    match: {
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    licPathRefsToMove.forEach(licPathRef => {
      ...
      if (!licPath) return;
      const listItem = plateCore.getParent(editor, licPath);
      if (!listItem) return;
      const listEntry = plateCore.getParent(editor, listItem[1]);
...
const insertListItem = editor => {
  ...
  const licEntry = plateCore.getAbove(editor, {
    match: {
      type: licType
    }
  });
  ...
  const listItemEntry = plateCore.getParent(editor, paragraphPath);
...
const moveListItemSublistItemsToListItemSublist = (editor, {
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    ...
    if (!toListItemSublist) {
      const fromList = plateCore.getParent(editor, fromListItemPath);
...
const removeFirstListItem = (editor, {
  ...
  if (!isListNested(editor, listPath) && !plateCore.isFirstChild(listItemPath)) {
    moveListItemUp(editor, {
      list,
      listItem
    });
...
const toggleList = (editor, {
  ...
  if (plateCore.isCollapsed(editor.selection) || !plateCore.isRangeAcrossBlocks(editor)) {
    ...
    if (res) {
      ...
      const nodes = [...plateCore.getNodes(editor, {
        match: {
          type: plateCore.getPluginType(editor, plateCore.ELEMENT_DEFAULT)
        }
      })];
...
const onKeyDownList = (editor, {
  ...
  if (e.key === 'Tab' && editor.selection) {
    const listSelected = plateCore.getAbove(editor, {
      at: editor.selection,
      match: {
        type
      }
...
const selectionIsNotInAListHandler = editor => {
  ...
  if (pointAfterSelection) {
    ...
    if (nextSiblingListRes) {
      ...
      if (!plateCore.getText(editor, parentBlockEntity[1])) {
...
const deleteFragmentList = editor => {
  ...
  slate.Editor.withoutNormalizing(editor, () => {
    ...
    const liEnd = plateCore.getAbove(editor, {
      at: end,
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_LI)
      }
    ...
    const liStart = plateCore.getAbove(editor, {
      at: start,
      match: {
        type: plateCore.getPluginType(editor, ELEMENT_LI)
      }
    ...
    if (liEndPathRef) {
      ...
      const listStart = liStart && plateCore.getParent(editor, liStart[1]);
...
const insertFragmentList = editor => {
  ...
  const li = plateCore.getPlugin(editor, ELEMENT_LI);
  const ul = plateCore.getPlugin(editor, ELEMENT_UL);
  const ol = plateCore.getPlugin(editor, ELEMENT_OL);
  ...
  const convertFragmentToList = fragment => {
    ...
    return trimmedFragment.map(node => {
      ...
      return {
        type: ELEMENT_LI,
        children: (_node$children = node.children) !== null && _node$children !== void 0 ? _node$children : [node]
      };
    });
  };

  return fragment => {
    const liEntry = plateCore.findNode(editor, {
      match: {
        type: li.type
      },
      mode: 'lowest'
    ...
    if (liEntry) {
      ...
      return slate.Transforms.insertNodes(editor, convertFragmentToList(fragment), {
...
const normalizeListItem = (editor, {
  ...
  if (slate.Editor.isBlock(editor, firstLiChildNode) && !plateCore.match(firstLiChildNode, {
    type: plateCore.getPluginType(editor, ELEMENT_LIC)
  })) {
    if (plateCore.match(firstLiChildNode, {
      ...
      const parent = plateCore.getParent(editor, listItem[1]);
...
const normalizeNestedList = (editor, {
  ...
  const parentNode = plateCore.getParent(editor, path);
...
const normalizeList = (editor, {
  ...
  return ([node, path]) => {
    ...
    if (node.type === licType && licType !== defaultType) {
      ...
      if (((_getParent = plateCore.getParent(editor, path)) === null || _getParent === void 0 ? void 0 : _getParent[0].type) !== liType) {
        plateCore.setNodes(editor, {
          type: defaultType
        }, {
          at: path
