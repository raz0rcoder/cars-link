...
const decorateCodeLine = (editor, {
  ...
  return entry => {
    ...
    const codeBlock = plateCore.getParent(editor, path);
    if (!codeBlock) return;
    let langName = '';
    ...
    if (plateCore.isElement(node) && node.type === type) {
...
const getCodeLineEntry = (editor, {
  ...
  if (at && plateCore.someNode(editor, {
    ...
    const codeLine = plateCore.getAbove(editor, {
      at,
      match: {
        type: getCodeLineType(editor)
      }
    }) || plateCore.getParent(editor, parentPath);
    if (!codeLine) return;
    const [codeLineNode, codeLinePath] = codeLine;
    if (plateCore.isElement(codeLineNode) && codeLineNode.type !== getCodeLineType(editor)) return;
    const codeBlock = plateCore.getParent(editor, codeLinePath);
    if (!codeBlock) return;
    return {
      codeBlock,
      codeLine
...
const deleteStartSpace = (editor, {
  ...
  const spaceText = plateCore.getText(editor, spaceRange);
...
const onKeyDownCodeBlock = editor => e => {
  if (e.key === 'Tab') {
    ...
    if (codeLines && codeLines !== null && codeLines !== void 0 && codeLines[0]) {
      ...
      const codeBlock = plateCore.getParent(editor, firstLinePath);
...
const getIndentDepth = (editor, {
  ...
  const [, codeLinePath] = codeLine;
  const text = plateCore.getText(editor, codeLinePath);
...
const toggleCodeBlock = editor => {
  ...
  if (!isActive) {
    ...
    for (const [, path] of nodes) {
      ...
      plateCore.setNodes(editor, codeLine, {
        at: path
      });
    }
  }
