...
const withHOC = (HOC, Component) => props => /*#__PURE__*/React.createElement(HOC, null, /*#__PURE__*/React.createElement(Component, props));
...
const createEventEditorPlugin = createPluginFactory({
  ...
  handlers: {
    onFocus: editor => () => {
      var _eventEditorActions$f;

      (_eventEditorActions$f = eventEditorActions.focus) === null || _eventEditorActions$f === void 0 ? void 0 : _eventEditorActions$f.call(eventEditorActions, editor.id);
    },
    onBlur: editor => () => {
      var _eventEditorActions$b;

      (_eventEditorActions$b = eventEditorActions.blur) === null || _eventEditorActions$b === void 0 ? void 0 : _eventEditorActions$b.call(eventEditorActions, editor.id);
    }
  }
});
...
const createPlateStore = (state = {}) => createStore(`plate-${state.id}`)({
  ...
  resetEditor: () => {
    ...
    _set.editor(withPlate(createEditor$1(), {
      id: state.id,
...
const getPlateId = id => {
  var _eventEditorSelectors, _eventEditorSelectors2, _eventEditorSelectors3;

  if (id) return id;
  const focus = (_eventEditorSelectors = eventEditorSelectors.focus) === null || _eventEditorSelectors === void 0 ? void 0 : _eventEditorSelectors.call(eventEditorSelectors);
  if (focus) return focus;
  const blur = (_eventEditorSelectors2 = eventEditorSelectors.blur) === null || _eventEditorSelectors2 === void 0 ? void 0 : _eventEditorSelectors2.call(eventEditorSelectors);
  if (blur) return blur;
  const last = (_eventEditorSelectors3 = eventEditorSelectors.last) === null || _eventEditorSelectors3 === void 0 ? void 0 : _eventEditorSelectors3.call(eventEditorSelectors);
  return last !== null && last !== void 0 ? last : 'main';
};
const usePlateId = id => {
  var _useEventEditorSelect, _useEventEditorSelect2, _useEventEditorSelect3;

  const focus = (_useEventEditorSelect = useEventEditorSelectors.focus) === null || _useEventEditorSelect === void 0 ? void 0 : _useEventEditorSelect.call(useEventEditorSelectors);
  const blur = (_useEventEditorSelect2 = useEventEditorSelectors.blur) === null || _useEventEditorSelect2 === void 0 ? void 0 : _useEventEditorSelect2.call(useEventEditorSelectors);
  const last = (_useEventEditorSelect3 = useEventEditorSelectors.last) === null || _useEventEditorSelect3 === void 0 ? void 0 : _useEventEditorSelect3.call(useEventEditorSelectors);
  if (id) return id;
  if (focus) return focus;
  if (blur) return blur;
  return last !== null && last !== void 0 ? last : 'main';
...
const platesStore = createStore('plate')({}).extendActions(set => ({
  /**
...
const usePlateEffects = ({
  ...
  useEffect(() => {
    if (!editor && enabled) {
      ...
      plateActions.editor((_ref = editorProp) !== null && _ref !== void 0 ? _ref : withPlate(createEditor$1(), {
        id,
        plugins: pluginsProp,
        disableCorePlugins
      }));
...
const withEditor = Component => withHOC(EditorProvider, Component);
...
const Plate = props => {
  ...
  const isReady = usePlatesSelectors.has(id); // Clear the state on unmount.
  ...
  return /*#__PURE__*/React.createElement(PlateContent, props);
};
...
export { CARRIAGE_RETURN, DOM_HANDLERS, DefaultLeaf, ELEMENT_DEFAULT, EditablePlugins, EditorProvider, EditorRefEffect, EditorRefPluginEffect, EditorStateEffect, KEY_DESERIALIZE_AST, KEY_DESERIALIZE_HTML, KEY_EVENT_EDITOR, KEY_INLINE_VOID, KEY_INSERT_DATA, LINE_FEED, NO_BREAK_SPACE, Plate, PlateContent, SPACE, TAB, ZERO_WIDTH_SPACE, applyDeepToNodes, cleanHtmlBrElements, cleanHtmlCrLf, cleanHtmlEmptyElements, cleanHtmlFontElements, cleanHtmlLinkElements, cleanHtmlTextNodes, copyBlockMarksToSpanChild, createDeserializeAstPlugin, createDeserializeHtmlPlugin, createDocumentNode, createElementWithSlate, createEventEditorPlugin, createHistoryPlugin, createInlineVoidPlugin, createInsertDataPlugin, createNode, createNodeHOC, createNodesHOC, createNodesWithHOC, createPlateEditor, createPlateStore, createPluginFactory, createPlugins, createReactPlugin, defaultsDeepToNodes, deleteFragment, deserializeHtml, deserializeHtmlElement, deserializeHtmlNode, deserializeHtmlNodeChildren, elementToHtml, escapeRegExp, eventEditorActions, eventEditorSelectors, eventEditorStore, findDescendant, findHtmlElement, findHtmlParentElement, findNode, findNodePath, flattenDeepPlugins, getAbove, getBlockAbove, getChildren, getHandler, getHtmlComments, getInjectedPlugins, getKeyByType, getKeysByTypes, getLastChild$1 as getLastChild, getLastChildPath, getLastNode, getMark, getNextSiblingNodes, getNode, getNodes, getParent, getPlateActions, getPlateEditorRef, getPlateId, getPlatePlugins, getPlateSelectors, getPlateStore, getPlugin, getPluginInjectProps, getPluginOptions, getPluginType, getPlugins, getPluginsByKey, getPointBefore, getPointFromLocation, getPointNextToVoid, getPreventDefaultHandler, getPreviousBlockById, getPreviousPath, getQueryOptions, getRangeBefore, getRangeFromBlockStart, getRenderNodeProps, getSelectionText, getSlateClass, getText, hasSingleChild, hexToBase64, htmlBodyToFragment, htmlBrToNewLine, htmlElementToElement, htmlElementToLeaf, htmlStringToDOMNode, htmlTextNodeToString, insertEmptyElement, insertNodes, isAncestor, isAncestorEmpty, isBlockAboveEmpty, isBlockTextEmptyAfterSelection, isCollapsed, isDefined, isDescendant, isElement, isEncoded, isEnd, isEventHandled, isExpanded, isFirstChild, isHtmlBlockElement, isHtmlComment, isHtmlElement, isHtmlFragmentHref, isHtmlText, isLastChild, isMarkActive, isNull, isOlSymbol, isPointAtWordEnd, isRangeAcrossBlocks, isRangeInSingleText, isSelectionAtBlockEnd, isSelectionAtBlockStart, isSelectionExpanded, isStart, isText, isTextByPath, isType, isUndefined, isUndefinedOrNull, isUrl, isWordAfterTrigger, leafToHtml, mapInjectPropsToPlugin, match, matchPredicate, mergeDeepPlugins, mergeDeepToNodes, mergeNodes, mockPlugin, moveChildren, normalizeDescendantsToDocumentFragment, onKeyDownToggleElement, onKeyDownToggleMark, overridePluginsByKey, parseHtmlDocument, parseHtmlElement, pipe, pipeDecorate, pipeDeserializeHtmlElement, pipeDeserializeHtmlLeaf, pipeHandler, pipeInjectProps, pipeInsertDataQuery, pipeInsertFragment, pipeOnChange, pipeRenderElement, pipeRenderLeaf, pipeTransformData, pipeTransformFragment, platesActions, platesSelectors, platesStore, pluginDeserializeHtml, pluginInjectProps, pluginRenderElement, pluginRenderLeaf, postCleanHtml, preCleanHtml, queryEditor, queryNode, removeHtmlNodesBetweenComments, removeHtmlSurroundings, removeMark, replaceTagName, selectEditor, selectEndOfBlockAboveSelection, serializeHtml, setDefaultPlugin, setMarks, setNodes, setPlatePlugins, someHtmlElement, someNode, stripClassNames, stripSlateDataAttributes, toggleMark, toggleNodeType, toggleWrapNodes, traverseHtmlComments, traverseHtmlElements, traverseHtmlNode, traverseHtmlTexts, trimWhitespace, unhangRange, unsetNodes, unwrapHtmlElement, unwrapNodes, useEditableProps, useEditorRef, useEditorState, useEventEditorSelectors, usePlate, usePlateEditorRef, usePlateEditorState, usePlateEffects, usePlateId, usePlatePlugins, usePlateSelection, usePlateSelectors, usePlateStore, usePlateStoreEffects, usePlatesSelectors, useSlateProps, withEditor, withHOC, withInlineVoid, withInsertData, withPlate, withProps, withProviders, wrapNodes };
